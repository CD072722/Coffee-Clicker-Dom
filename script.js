...
function getUnlockedProducers(data) {
  // your code here
  const showUnlocked = data.producers.filter(
    (element) => element.unlocked === true
  );
  return showUnlocked;
}
...
function getProducerById(data, producerId) {
  // your code here
  let producer = data.producers.filter((element) => element.id === producerId);
  return producer[0];
}
